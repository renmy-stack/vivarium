...
const w = V.createWorld(seed); w.settings.food = food;
const t0 = Date.now();
let extAt = [];
console.log('分   人口  草  肉 | 大きさ 速さ 目  肉寄り | 肉食 草食 | 世代 | 種');
for (let m = 1; m <= minutes; m++) {
  ...
  if (m % 5 === 0 || m === 1) {
    ...
    console.log(String(m).padStart(3) + ' ' + String(s.pop).padStart(4) + ' ' + String(s.grass).padStart(3) + ' ' + String(s.meat).padStart(3) + ' | ' + s.size.toFixed(1).padStart(5) + ' ' + s.speed.toFixed(2) + ' ' + s.sight.toFixed(0).padStart(3) + ' ' + s.diet.toFixed(2) + '  | ' + String(s.carn).padStart(3) + ' ' + String(s.herb).padStart(4) + ' | ' + String(s.maxGen).padStart(3) + '  | ' + sp.length + ' (' + sp.slice(0, 3).map(g => g.n + '匹 大' + g.size.toFixed(0) + ' 速' + g.speed.toFixed(1) + ' 肉' + g.diet.toFixed(1)).join(', ') + ')');
  }
}
...
console.log('URL ' + enc.length + ' 文字 往復 ' + (dec && dec.creatures.length === Math.min(120, w.creatures.length) && dec.maxPop === w.stats.maxPop ? 'OK' : 'NG'));
// 決定性
